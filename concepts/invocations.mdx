...
```bash
# CLI
cast invoke my-agent "Complex task" --timeout 300000

# SDK
await client.agents.invoke('my-agent', 'Complex task', { timeout: 300000 });
```
...
```bash
cast invoke my-agent "Write a long response" --stream
```
...
Shows full input, output, and error details.
...
const results = await Promise.all([
  client.agents.invoke('my-agent', 'Task 1'),
  client.agents.invoke('my-agent', 'Task 2'),
  client.agents.invoke('my-agent', 'Task 3'),
]);
```
...
# Run 'cast logs my-agent --invocation inv_xyz' for details
```
...
<CardGroup cols={2}>
  ...
  <Card title="Agents API" icon="code" href="/sdk/agents#invokeslug-prompt">
    SDK reference
  </Card>
